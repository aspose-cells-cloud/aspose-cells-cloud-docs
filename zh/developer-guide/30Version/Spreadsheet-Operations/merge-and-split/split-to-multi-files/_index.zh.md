...
linktitle: Split Multi Excel 文件
type: docs
url: /zh/split-an-excel-file-to-multi-files/
aliases: [/split-excel-workbooks/,/workbook/split/]
keywords: Split multi Excel files
description: Aspose.Cells Cloud REST API 支持拆分多个 Excel 文件。SDK 支持多种开发语言，包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 Swift。
weight: 32
kwords: Excel, Office 云, REST API, 电子表格, PDF, CSV, Json, Markdown, Split
---
此 REST Api 指示 `split` 个多个 Excel 文件。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/LightCells/PostSplit)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
