---
title: 在 Excel 文件中添加水印
second_title: Documen
linktitle: 在 Excel 文件中添加水印
type: docs
url: /zh/add-watermark-into-excel-files/
aliases: [ /watermark/]
keywords: Add water marker for Excel files
description: Aspose.Cells Cloud REST API 支持为 Excel 文件添加水印。SDK 支持多种开发语言，包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 Swift。
weight: 39
kwords: Excel, Office 云, REST API, 电子表格, PDF, CSV, Json, Markdown, 水印
---
此 REST API 表示在 Excel 文件上添加 `watermark`。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/LightCells/PostWatermark)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
