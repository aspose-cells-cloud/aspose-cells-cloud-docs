...
这[OpenAPI规范](https://reference.aspose.cloud/cells/#/Conversion/PostConvertWorkbookToPDF)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
...
[POST /cells/{name}/saveAs](https://apireference.aspose.cloud/cells/#/SaveAs/PostDocumentSaveAs) API 可让您将 MS Excel 文件保存为带有附加设置的 PDF 文件，并将结果保存到存储中。
