...
| sheetName     | string | path   | 待重命名工作表的当前名称。           |
| newname       | string | query  | 工作表的新名称。                     |
| folder        | string | query  | 存储中的文件夹路径（可选）。         |
| storageName   | string | query  | 存储的名称（可选）。                 |

[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/Worksheets/PostRenameWorksheet) 定义了一个公开可用的编程接口，可让您直接从 Web 浏览器发起 REST 请求。
