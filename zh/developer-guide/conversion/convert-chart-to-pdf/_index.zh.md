---
title: Aspose.Cells 云网络 API - 将电子表格图表转换为 PDF 文件
...
description: 此 API 将位于本地驱动器上的电子表格中的图表无缝转换为 PDF 文件
weight: 100
kwords: Excel、Office 云、REST API、电子表格转换、PDF、CSV、JSON、Markdown、将图表转换为 PDF、云转换
---
将图表从本地电子表格/Excel 文件转换为[PDF](https://docs.fileformat.com/pdf/)文件。
...
|工作表|细绳|询问|包含图表的工作表的名称。|
|图表索引|整数|询问|要转换的图表的索引。|
|输出路径|细绳|询问|（可选）转换后文件的存储文件夹路径。默认值为 null。|
...
## 您应该在哪里使用将图表转换为 PDF API？

- 将电子表格中的图表导出为 Pdf。

## 为什么要使用将图表转换为 PDF API？

- 无需云存储，减少云资源负担。
- 通过现有的SDK即可快速完成开发。

## 如何使用 SDK 将图表转换为 PDF API？

### 将图表转换为 PDF API 规格

这[将图表转换为 PDF API 规格](https://reference.aspose.cloud/cells/#/ConversionController/ConvertChartToPdf)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。

## 使用 Aspose.Cells 云 SDK

使用 SDK 是最快的开发方式，因为它抽象了低级细节，允许您使用短代码将图表转换为 pdf 文件。
