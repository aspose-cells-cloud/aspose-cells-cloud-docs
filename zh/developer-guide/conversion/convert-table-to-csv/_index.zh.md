---
title: Aspose.Cells Cloud Web API - 将电子表格数据转换为 Csv 文件
...
description: 使用我们的 API 将本地驱动器上的电子表格中的表格高效转换为 CSV 文件
weight: 100
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、JSON、Markdown、表格转 CSV、本地转云
---
将本地电子表格/Excel 表转换为 csv 文件。
...
## 为什么要使用将表格转换为 Csv API？

- 无需云存储，减少云资源负担。
- 通过现有的SDK即可快速完成开发。

## 如何使用 SDK 将表格转换为 Csv API？

### 将表格转换为 Csv API 规范

这[将表格转换为 Csv API 规范](https://reference.aspose.cloud/cells/#/ConversionController/ConvertTableToCsv)提供一个可公开访问的编程接口，允许直接从 Web 浏览器进行 REST 交互。

### 使用 Aspose.Cells 云 SDK

使用 SDK 是最快的开发方式，因为它抽象了低级细节，允许您使用短代码将电子表格数据转换为 csv 文件。
