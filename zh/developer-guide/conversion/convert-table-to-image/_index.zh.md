---
title: Aspose.Cells Cloud Web API - 将电子表格数据转换为图像
...
description: 使用 Aspose.Cells Cloud Web API 将本地电子表格高效转换为图像文件
weight: 100
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、JSON、Markdown、将表格转换为图像、云转换、图像文件格式、电子表格转换
---
将本地电子表格/Excel表格数据转换为图像。支持**图像格式：** [PNG](https://docs.fileformat.com/image/png/), [SVG](https://docs.fileformat.com/page-description-language/svg/), [TIFF](https://docs.fileformat.com/image/tiff/), [JPEG](https://docs.fileformat.com/image/jpeg/), [BMP](https://docs.fileformat.com/image/bmp/)
...
|表名|细绳|询问|要转换的表的名称。|
|格式|细绳|询问|所需的图像文件格式（例如，png、svg）。|
|输出路径|细绳|询问|（可选）转换后图片的存储文件夹路径。默认为 null。|
|输出存储名称|细绳|询问|指定输出文件存储名称。|
|字体位置|细绳|询问|如果需要，请使用自定义字体。|
|地区|细绳|询问|定义电子表格区域设置。|
|密码|细绳|询问|访问电子表格文件需要密码。|
...
## 如何使用 SDK 将表格转换为图像 API？

### 将表格转换为图像 API 规格

这[将表格转换为图像 API 规格](https://reference.aspose.cloud/cells/#/ConversionController/ConvertTableToImage)提供可公开访问的编程接口，用于直接从 Web 浏览器进行 REST 交互。

### 使用 Aspose.Cells 云 SDK

使用 SDK 是最快的开发方式，因为它抽象了低级细节，允许您使用短代码将电子表格数据转换为图像。
