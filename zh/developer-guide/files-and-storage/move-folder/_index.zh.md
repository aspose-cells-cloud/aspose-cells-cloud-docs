...
description: 本文档提供了使用移动文件夹 API 管理 Aspose.Cells 云存储中的文件夹的综合指南
weight: 100
kwords: Excel API，移动文件夹 API，Office 云，REST API，电子表格管理，PDF，CSV，JSON，Markdown，匹配工作表中的所有空白单元格 Excel
---
## **Excel API：移动文件夹**
...
此 API 允许用户在 Aspose.Cells 云存储中将文件夹从一个位置移动到另一个位置。这对于有效地组织文件和管理云存储至关重要。

### 请求参数**移动文件夹**API 是

|参数名称|类型|路径/查询字符串/HTTP 正文|描述|
|----------------|--|------------------------|--------------------------------------------------------|
|源路径|细绳|小路|要移动的文件夹的源路径。|
|目标路径|细绳|询问|文件夹应移动到的目标路径。|
|源存储名称|细绳|询问|源存储的名称。|
...
这[OpenAPI 规范](https://reference.aspose.cloud/cells/#/FolderController/MoveFolder)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
