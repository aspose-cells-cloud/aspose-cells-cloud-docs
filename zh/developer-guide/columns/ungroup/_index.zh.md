...
description: Aspose.Cells Cloud REST API 支持在 Excel 工作表上取消分组列。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 70
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、取消分组 Excel 工作表上的列
---
...
|姓名|细绳|小路|工作簿名称。|
|工作表名称|细绳|小路|工作表名称。|
|第一个索引|整数|询问|第一个要操作的列索引。|
|最后索引|整数|询问|需要操作的最后一列索引。|
|文件夹|细绳|询问|文檔夹。|
|存储名称|细绳|询问|存储名称。|
