...
description: Aspose.Cells Cloud REST API 支持在 Excel 工作表上解冻窗格。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 200
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、取消冻结 Excel 工作表上的窗格
---
此 REST API 表示 Excel 工作表上的 `unfreeze panes`。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/Worksheets/DeleteWorksheetFreezePanes)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
