...
description: Aspose.Cells Cloud REST API 支持将工作表转换为与 Excel 工作表不同的格式。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 10
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、从 Excel 工作表获取工作表
---
这个 REST API 表示 `get worksheet description` 或 `export worksheet to kinds of format file`。
