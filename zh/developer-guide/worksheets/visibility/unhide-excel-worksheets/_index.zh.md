...
description: Aspose.Cells Cloud REST API 支持在 Excel 工作簿上取消隐藏 Excel 工作表。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 60
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、取消隐藏 Excel 工作表
---
这个 REST API 表示 `unhide worksheet`。
...
|姓名|细绳|小路|文档名称。|
|工作表名称|细绳|小路|工作表名称。|
|是否可见|布尔值|询问|新的工作表可见性值。|
|文件夹|细绳|询问|文檔夹。|
|存储名称|细绳|询问|存储名称。|
 
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/Worksheets/PutChangeVisibilityWorksheet)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
