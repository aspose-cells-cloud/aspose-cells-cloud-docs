...
description: Aspose.Cells云API支持批量转换多个excel文件。SDK支持多种开发语言。包括Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby和swift
weight: 100
kwords: Excel, Office 云, REST API, 电子表格, PDF, CSV, Json, Markdwon, 批量转换
---
此 REST API 表示符合条件的文件为 `batch conversion`
...
名称 | 类型 | 描述 | 注释
------------ | ------------- | ------------- | -------------
SourceFolder | 字符串 | | [可选]MatchCondition | MatchConditionRequest | | [可选]Format | 字符串 | | [可选]OutFolder | 字符串 | | [可选]SaveOptions | SaveOptions | | [可选]**MatchConditionRequest 属性**
 
名称 | 类型 | 描述 | 注释
------------ | ------------- | ------------- | -------------
RegexPattern | 字符串 | | [可选]FullMatchConditions | 字符串[]| | [可选][OpenAPI 规范](https://apireference.aspose.cloud/cells/#/PostBatchConvert)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
