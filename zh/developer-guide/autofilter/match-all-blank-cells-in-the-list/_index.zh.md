...
description: Aspose.Cells Cloud API 支持匹配 Excel 工作表上的筛选列表中的所有空白单元格。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 100
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、匹配 Excel 工作表中的所有空白单元格
---
此 REST API 向 `match` 指示 Excel 工作表上的过滤器列表中的所有 `blank cell`。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/AutoFilter/PostWorksheetMatchBlanks)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
