---
title: 在 Excel 工作表中获取自动过滤器
second_title: Aspose.Cells Cloud Documen
linktitle: 获取自动过滤
type: docs
url: /zh/autofilter/get/
aliases: [/get-autofilter-description/]
keywords: Gets auto filter description from an Excel worksheet
description: Aspose.Cells Cloud API 支持从 Excel 工作表获取自动筛选描述。SDK 支持多种开发语言。其中包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 50
kwords: Excel、Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、在 Excel 工作表中获取自动过滤器
---
此 REST API 表示在 Excel 工作表中获取 `auto filter` 描述。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/AutoFilter/GetWorksheetAutoFilter)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
