...
description: Aspose.Cells云API支持匹配Excel工作表上过滤列表中的所有非空白单元格。SDK支持多种开发语言。它们包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 100
---
此 REST API 指示 Excel 工作表上的列表中的 `match` 所有 `not blank cell`。
...
这[开放API规范](https://apireference.aspose.cloud/cells/#/AutoFilter/PostWorksheetMatchNonBlanks)定义了一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
