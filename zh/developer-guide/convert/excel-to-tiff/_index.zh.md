...
description: Aspose.Cells Cloud REST API 支持将excel文件转换为tiff文件。 SDK支持多种开发语言。它们包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 90
---
此 REST API `saveas` excel 文件为 TIFF。

[POST /cells/{name}/saveAs](https://apireference.aspose.cloud/cells/#/SaveAs/PostDocumentSaveAs) API 允许您将 MS Excel 文件另存为带有附加设置的 TIFF 文件，并将结果保存到存储中。

此 REST API `convert` excel 文件为 TIFF。
...
此 REST API `export` excel 文件为 TIFF。

[获取/单元格/{名称}](https://apireference.aspose.cloud/cells/#/Workbook/GetWorkBook  )API 允许您使用附加设置将 MS Excel 文件转换为 TIFF 文件，并将结果保存到响应中。
