...
type: docs
url: /zh/convert/excel-to-tiff/
aliases: [/convert-excel-file-to-tiff-in-cloud/]
keywords: Convert excel files to tiff files
description: Aspose.Cells Cloud REST API 支持将 excel 文件转换为 tiff 文件。SDK 支持多种开发语言。它们包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 90
kwords: Excel, Office 云、REST API、电子表格、PDF、CSV、Json、Markdwon、Excel 至 TIFF
---
此 REST API `saveas` excel 文件至 TIFF。

[POST /单元格/{名称}/saveAs](https://apireference.aspose.cloud/cells/#/SaveAs/PostDocumentSaveAs) API 可让您将 MS Excel 文件保存为 TIFF 文件并使用附加设置将结果保存到存储中。

此 REST API `convert` excel 文件至 TIFF。

[PUT /单元格/转换](https://apireference.aspose.cloud/cells/#/Workbook/PutConvertWorkBook) API 允许您使用附加设置将 MS Excel 文件转换为 TIFF 文件，并将结果保存到响应中。

此 REST API `export` excel 文件至 TIFF。

[获取 /cells/{名称}](https://apireference.aspose.cloud/cells/#/Workbook/GetWorkBook  ) API 允许您使用附加设置将 MS Excel 文件转换为 TIFF 文件，并将结果保存到响应中。
