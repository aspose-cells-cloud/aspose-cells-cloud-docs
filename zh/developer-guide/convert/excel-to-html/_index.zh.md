...
linktitle: Excel 转 HTM
type: docs
url: /zh/convert/excel-to-html/
aliases: [/convert-excel-file-to-html-in-cloud/]
keywords: Convert excel files to html files
description: Aspose.Cells Cloud REST API 支持将excel文件转换为html文件。 SDK支持多种开发语言。它们包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 100
---
此 REST API `saveas` excel 文件为 HTML。

[POST /cells/{name}/saveAs](https://apireference.aspose.cloud/cells/#/SaveAs/PostDocumentSaveAs) API 允许您将 MS Excel 文件另存为带有附加设置的 HTML 文件，并将结果保存到存储中。

此 REST API `convert` excel 文件为 HTML。
...
此 REST API `export` excel 文件为 HTML。

[获取/单元格/{名称}](https://apireference.aspose.cloud/cells/#/Workbook/GetWorkBook  )API 允许您使用附加设置将 MS Excel 文件转换为 HTML 文件，并将结果保存到响应中。
