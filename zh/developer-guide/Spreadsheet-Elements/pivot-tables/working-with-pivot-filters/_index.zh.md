...
description: Aspose.Cells Cloud REST API 支持为数据透视表添加筛选器。SDK 支持多种开发语言，包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 Swift。
weight: 50
kwords: Excel, Office 云, REST API, 电子表格, PDF, CSV, Json, Markdown, 使用数据透视表筛选器
---
此 REST API 表示 `add` 枢轴 `filter` 用于数据透视表索引
...
这[OpenAPI规范](https://apireference.aspose.cloud/cells/#/PivotTables/PutWorksheetPivotTableFilter)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
