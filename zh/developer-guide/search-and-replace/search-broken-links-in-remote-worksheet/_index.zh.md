...
description: 使用 Excel API 轻松搜索远程电子表格工作表中的断开链接
weight: 100
kwords: Excel API, Office 云, REST API, 电子表格, PDF, CSV, JSON, Markdown, 断开的链接, 无效的 URL, 超链接验证
---
在远程电子表格工作表中搜索断开的链接。
...
## 我们应该在哪里使用电子表格 API 工作表中的“搜索断开的链接”功能？

当您需要在电子表格的工作表中搜索断开的链接时，您可以使用这个 API。

## 为什么要使用电子表格 API 工作表中的“搜索断开的链接”功能？

- 使用此 API 轻松搜索远程电子表格工作表中的断开链接。
- 通过现有的SDK即可快速完成开发。
...
这[OpenAPI 规范](https://reference.aspose.cloud/cells/#/SearchControllor/SearchBrokenLinksInRemoteWorksheet)定义一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。

### 使用 Aspose.Cells 云 SDK

使用 SDK 是加速开发的最佳方式。SDK 处理底层细节，让您能够以最少的代码轻松实现在电子表格工作表内搜索单元格。
