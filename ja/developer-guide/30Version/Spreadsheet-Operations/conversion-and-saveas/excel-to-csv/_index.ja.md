...
keywords: "Excel to CSV, Aspose.Cells Cloud, REST API, スプレッドシート変換, CSV ファイル, ファイル変換"
description: "Aspose.Cells Cloud REST API を使用して Excel スプレッドシートを CSV に変換します。複数の SDK とプログラミング言語をサポートし、簡単に統合できます。"
weight: 90
---

この REST API は、スプレッドシートファイルを CSV 形式のファイルに変換します。
...
| `checkExcelRestriction` | bool   | ユーザーがセル関連のオブジェクトを変更する際に、Excel ファイルの制限をチェックするかどうか。 |

### リクエストボディパラメータ

| パラメータ名 | タイプ      | 説明                                                       |
| ------------ | --------- | ---------------------------------------------------------- |
| `datafile`   | data file | マルチパートリクエストボディの最初のパートに含まれるデータファイル。 |

### レスポンス

API は、生成された CSV ファイルを含む **FileInfo** オブジェクトを返します。

| フィールド        | タイプ   | 説明                                |
| --------------- | ------ | ----------------------------------- |
| **Filename**    | string | CSV ファイル名 (例: `example.csv`) |
| **FileSize**    | int    | ファイルサイズ（バイト単位）。        |
| **FileContent** | string | CSV ファイルの Base64 エンコードされた内容。 |
...
## SDK を使用した PostConvertWorkbookToCSV API の利用方法

### PostConvertWorkbookToCSV API の仕様

[OpenAPI 仕様](https://reference.aspose.cloud/cells/#/Conversion/PostConvertWorkbookToCSV) は、パブリックに利用可能なプログラミングインタフェースを定義し、Web ブラウザから直接 REST 操作を実行できるようにします。

**cURL** コマンドラインツールを使用して Aspose.Cells Web サービスにアクセスできます。以下の例は、cURL を使用して Cloud API を呼び出す方法を示しています。
...
SDK を使用すると、最も迅速に開発できます。SDK が低レベルの詳細を処理するため、プロジェクトに集中できます。Aspose.Cells Cloud SDK の完全な一覧については、[GitHub リポジトリ](https://github.com/aspose-cells-cloud) を参照してください。
