...
このREST APIは、Excelファイル**内に**データを統合します。
...
| パラメータ名 | 型     | 位置                    | 説明                                           |
| ------------ | ------ | ----------------------- | ---------------------------------------------- |
| file         | file   | formData（multipart body） | アップロードするスプレッドシートファイル。        |
| DataSource   | string | クエリ文字列            | 統合用データを提供するデータソースの識別子。     |
| format       | string | クエリ文字列            | 出力フォーマット（例: `xlsx`、`pdf`）。         |
...
| 401  | Unauthorized        | JWTトークンが無効、または不足しています。           |
| 413  | Payload Too Large   | アップロードされたファイルがサイズ制限を超えています。 |
| 500  | Internal Server Error | サーバーで予期しないエラーが発生しました。           |

## SDKを使用したPostAssemble APIの使い方

### PostAssemble API仕様

[OpenAPI仕様](https://apireference.aspose.cloud/cells/#/LightCells/PostAssemble)は、パブリックにアクセス可能なプログラミングインターフェースを定義し、Webブラウザから直接RESTのやり取りを実行できるようにします。

cURLコマンドラインツールを使用すると、Aspose.Cells Webサービスに簡単にアクセスできます。以下の例は、cURLを使用してCloud APIへの呼び出しを行う方法を示しています。
...
SDKを使用すると、APIに対して最も迅速に開発が可能です。SDKは低レベルの詳細を抽象化し、ビジネスロジックに集中できるようにします。Aspose.Cells Cloud SDKの完全な一覧については、[GitHubリポジトリ](https://github.com/aspose-cells-cloud)をご確認ください。
