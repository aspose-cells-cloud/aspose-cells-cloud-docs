...
ArticleTitle: "Aspose.Cells Cloud API – ExcelワークシートからMaxDataRowを取得する"
---

このREST APIは、`cellOrMethodName`パラメータが`maxdatarow`に設定された場合、Excelファイル内の最大データ行インデックスを返します。
...
*注: リクエストは**HTTPS**経由で送信され、有効なOAuth2ベアラートークンを含める必要があります。*
...
| コード | 説明 |
|------|-------------|
| 200 | 成功 – 最大データ行インデックスを返します。 |
| 401 | 認証エラー – 無効または不足している認証トークン。 |
| 403 | アクセス拒否 – ワークブックにアクセスするための権限が不足しています。 |
| 404 | 見つかりません – 指定されたワークブックまたはワークシートが存在しません。 |
| 500 | サーバーエラー – サーバー側で予期しない状況が発生しました。 |
...
SDKを使用することは、開発を最適に高速化する最も効率的な方法です。SDKが低レベルの詳細を処理するため、プロジェクトのタスクに集中できます。Aspose.Cells Cloud SDKの完全な一覧については、[GitHubリポジトリ](https://github.com/aspose-cells-cloud)をご確認ください。
...
- <a href="https://docs.aspose.cloud/cells/get-maxrow-from-excel-worksheet/" target="_blank" rel="noopener noreferrer">ExcelワークシートからMaxRowを取得する</a>  
- <a href="https://docs.aspose.cloud/cells/get-maxcolumn-from-excel-worksheet/" target="_blank" rel="noopener noreferrer">ExcelワークシートからMaxColumnを取得する</a>  
- <a href="https://docs.aspose.cloud/cells/get-mindatarow-from-excel-worksheet/" target="_blank" rel="noopener noreferrer">ExcelワークシートからMinDataRowを取得する</a>
...
<script type="application/ld+json">
{
  ...
  "description": "指定されたワークシート内にデータが含まれる最終行のインデックスを返します。",
  ...
  "input": [
    {
      "name": "fileName",
      "valueRequired": true,
      "description": "Excelワークブックの名前。"
    },
    {
      "name": "sheetName",
      "valueRequired": true,
      "description": "ワークシートの名前。"
    }
  ],
  "output": {
    "@type": "DataType",
    "name": "MaxDataRow",
    "description": "データを含む最終行の0始まりのインデックス。"
  }
}
</script>
