...
この REST API は、ワークシートから条件付き書式ルールを取得します。
...
| sheetName        | string  | path   | 書式が含まれるワークシートの名前。                |
| index            | integer | path   | 条件付き書式ルールの 0 から始まるインデックス。    |
...
| HTTP コード | 理由                                                 | 例（ボディ）                                                        |
| ----------- | ---------------------------------------------------- | ------------------------------------------------------------------- |
| **400**     | リクエストエラー – パラメータが不足または無効です。  | `{ "Code":"400", "Message":"Invalid parameter value." }`            |
...
[OpenAPI スペック](https://apireference.aspose.cloud/cells/#/ConditionalFormattings/GetWorksheetConditionalFormatting) は公開可能なプログラミングインターフェースを定義しており、Web ブラウザから直接 REST アクセスを実行できます。
...
SDK を使用すると、開発速度を最大限に高めることができます。SDK が低レベルの詳細を処理するため、開発者はプロジェクトのタスクに集中できます。Aspose.Cells Cloud SDK の完全な一覧については、[GitHub リポジトリ](https://github.com/aspose-cells-cloud) をご覧ください。
