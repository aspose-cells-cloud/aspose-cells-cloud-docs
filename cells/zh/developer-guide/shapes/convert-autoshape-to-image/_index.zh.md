...
linktitle: 转换
type: docs
url: /zh/shapes/conversion/
aliases: [/convert-autoshape-to-image/]
keywords: Convert shape on an Excel workshee
description: Aspose.Cells Cloud REST API 支持在 Excel 工作表上转换形状。 SDK支持多种开发语言。它们包括 Android、C#、Go、Java、NodeJS、Perl、PHP、Python、Ruby 和 swift
weight: 10
---
此 REST API 指示在 Excel 工作表上获取具有图像格式的形状。
...
这[OpenAPI 规范](https://apireference.aspose.cloud/cells/#/Shapes/GetWorksheetShape)定义了一个可公开访问的编程接口，并允许您直接从 Web 浏览器执行 REST 交互。
