---
title: Aspose.Cells Cloud Web API - Exporter les données d'un tableau de feuille de calcul vers un fichier de format
second_title: Documen
ArticleTitle: Export a Spreadsheet Table data as a Format file
linktitle: Exporter le tableau vers le format spécifié
type: docs
...
description: Convertit efficacement les tableaux des feuilles de calcul du stockage cloud vers divers formats spécifiés
weight: 100
kwords: Conversion de feuille de calcul, PDF, CSV, Excel, JSON, Markdown, Exporter le tableau
---
Exporter une feuille de calcul cloud/tableau Excel vers un autre fichier au format.

## **Exporter le tableau au format API**
...
|Nom de la table|Chaîne|Chemin|Nom de la table.|
|format|Chaîne|Requête|(Obligatoire) Le format souhaité (par exemple, « png », « pdf », « svg »).|
...
|chemin de sortie|Chaîne|Requête|(Facultatif) Chemin d'accès au dossier de stockage des données de sortie. La valeur par défaut est null.|
|outStorageName|Chaîne|Requête|Nom du stockage du fichier de sortie.|
|Emplacement des polices|Chaîne|Requête|Emplacement pour les polices personnalisées.|
|région|Chaîne|Requête|Paramètre pour la région de la feuille de calcul.|
...
## Pourquoi devriez-vous utiliser le tableau de feuille de calcul d'exportation au format API ?
...
## Comment utiliser le tableau d'exportation de feuille de calcul au format API avec les SDK ?

### Exporter le tableau au format API Spécification

 Le[Exporter le tableau au format API Spécification](https://reference.aspose.cloud/cells/#/ConversionController/ExportTableAsFormat) définit une interface de programmation accessible au public et vous permet d'effectuer des interactions REST directement à partir d'un navigateur Web.

### Utiliser les SDK Cloud Aspose.Cells

L'utilisation du SDK est le moyen le plus rapide de développer, car il fait abstraction des détails de bas niveau, vous permettant d'exporter les données d'une table de feuille de calcul vers un fichier de format avec un code court.
