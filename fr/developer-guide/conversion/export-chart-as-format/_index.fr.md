---
title: Aspose.Cells Cloud Web API - Exporter un graphique de feuille de calcul au format de fichier
second_title: Documen
ArticleTitle: Export a Spreadsheet Chart as a Format fil
linktitle: Exporter le graphique au format Forma
type: docs
...
description: Convertit efficacement les graphiques des feuilles de calcul stockées dans le cloud vers des formats spécifiés tels que PDF ou une image directement sans téléchargement
weight: 100
kwords: Exporter un graphique, REST, Conversion de feuille de calcul, PDF, CSV, JSON, Markdown, Excel, Format d'image
---
Exportez une feuille de calcul cloud/un graphique Excel vers un autre fichier de format avec le Cloud Web Aspose.Cells API.

## **Exporter le graphique au format API**
...
| feuille de travail| Chaîne| Chemin| Nom de la feuille de calcul|
| index des graphiques| Entier| Chemin| Index des graphiques|
| format| Chaîne| Requête| (Obligatoire) Le format de sortie souhaité (par exemple, « png », « pdf », « svg »).|
| dossier| Chaîne| Requête| (Facultatif) Le chemin du dossier dans lequel le classeur est stocké ; la valeur par défaut est null.|
| nom de stockage| Chaîne| Requête|(Facultatif) Le nom du stockage si vous utilisez un stockage cloud personnalisé ; utilisez le stockage par défaut s'il est omis.|
...
## Pourquoi devriez-vous utiliser le graphique de feuille de calcul d'exportation au format API ?
...
## Comment utiliser le graphique de feuille de calcul d'exportation au format API avec les SDK ?

### Exporter le graphique au format API Spécification

 Le[Exporter le graphique au format API Spécification](https://reference.aspose.cloud/cells/#/ConversionController/ExportChartAsFormat) définit une interface de programmation accessible au public, vous permettant d'effectuer des interactions REST directement à partir d'un navigateur Web.

### Utiliser les SDK Cloud Aspose.Cells

L'utilisation du SDK est le moyen le plus rapide de développer, car il fait abstraction des détails de bas niveau, vous permettant d'exporter les données d'un graphique de feuille de calcul vers un fichier de format avec un code court.
