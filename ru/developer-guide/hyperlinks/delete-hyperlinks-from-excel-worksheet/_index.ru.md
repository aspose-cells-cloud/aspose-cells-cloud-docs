...
description: Aspose.Cells Облачный REST API поддерживает удаление гиперссылок из рабочего листа Excel. SDK поддерживает различные языки разработки. К ним относятся Android, C#, Go, Java, NodeJS, Perl, PHP, Python, Ruby и swift.
weight: 40
---
Этот REST API указывает на `delete all hyperlinks` на листе Excel.
...
[Спецификация OpenAPI](https://apireference.aspose.cloud/cells/#/Hypelinks/DeleteWorksheetHyperlinks) определяет общедоступный программный интерфейс и позволяет выполнять REST-взаимодействия непосредственно из веб-браузера.
