...
| тип диаграммы| нить| запрос| Тип диаграммы см. в свойстве Тип ресурса диаграммы.|
...
| область| нить| запрос| Указывает значения, на основе которых будет построен ряд данных.|
| isVertical| булев| запрос| Истинный|
| категорияДанные| нить| запрос|Возвращает или задаёт диапазон значений оси категорий. Это может быть диапазон ячеек (например, «d1:e10»).|
...
| dataLabels| булев| запрос| Истинный|
| dataLabelsPosition| нить| запрос| Выше|
| pivotTableSheet| нить| запрос||
| pivotTableName| нить| запрос||
 
 The[Спецификация OpenAPI](https://apireference.aspose.cloud/cells/#/Charts/PutWorksheetAddChart) определяет общедоступный программный интерфейс и позволяет осуществлять REST-взаимодействие непосредственно из веб-браузера.
