...
weight: 30
kwords: Excel, Office Облако, REST API, электронная таблица, PDF, CSV, Json, Markdwon, получить последнюю ячейку рабочего листа Excel
---
Этот REST API указывает, что показывает получение `endcell` в файле Excel, когда параметр `cellOrMethodName` равен `endcell`.
