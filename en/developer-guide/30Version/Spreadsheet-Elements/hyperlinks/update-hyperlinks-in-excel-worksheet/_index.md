...
_Updates the hyperlink identified by `hyperlinkIndex` in the worksheet `sheetName` of the file `name`._
...
_Tip:_ Save the JSON payload to a file (e.g., `payload.json`) and reference it with `--data @payload.json` for cleaner copy‑pasting.
...
_All SDKs are open‑source and can be found in the [Aspose.Cells Cloud GitHub repository](https://github.com/aspose-cells-cloud)._
