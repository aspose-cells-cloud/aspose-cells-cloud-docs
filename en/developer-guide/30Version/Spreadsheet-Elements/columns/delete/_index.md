...
| Code | Meaning |
|------|---------|
| **InvalidParameter** | One or more supplied parameters are malformed or out of range. |
| **FileNotFound** | The specified workbook or folder cannot be located. |
| **WorksheetNotFound** | The worksheet name does not exist in the workbook. |
| **AuthenticationFailed** | JWT token is missing, expired, or invalid. |
| **InternalError** | An unexpected server‑side exception occurred. |
