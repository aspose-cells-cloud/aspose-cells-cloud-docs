...
description: "Use the Aspose.Cells Cloud REST API to autofit multiple rows in an Excel worksheet. The API is available through various SDKs (C#, Java, PHP, Python, Ruby, Go, etc.) for seamless integration."
weight: 40
---

This REST API autofits rows on an Excel worksheet.
...
| **autoFitterOptions**| object | body                         | Options that control how rows are autofitted (e.g., ignore hidden rows).   |
| **startRow**        | integer | query                        | The first row to autofit (1‑based index).                                   |
| **endRow**          | integer | query                        | The last row to autofit (inclusive).                                        |
| **onlyAuto**        | boolean | query                        | If **true**, only automatic row height is applied; otherwise, full autofit is performed. |
...
```bash
curl -v "http://api.aspose.cloud/v3.0/cells/sampleAutoFit.xlsx/worksheets/Sheet1/autofitrows?startRow=1&endRow=7" \
  -X POST \
