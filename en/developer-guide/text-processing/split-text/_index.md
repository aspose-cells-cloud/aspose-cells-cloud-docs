...
All requests must include a valid OAuth 2.0 access token in the `Authorization` header (`Bearer {access_token}`). Obtain the token using the client‑credentials flow with your Aspose Cloud `client_id` and `client_secret`.
