...
Convert Excel text case instantly—switch between uppercase, lowercase, proper case (capitalize each word), or sentence case (capitalize the first letter of each sentence). Use Aspose.Cells Cloud Web API to adjust text formatting in bulk across your spreadsheet.

### Quick‑Start Summary
1. **Upload** the spreadsheet file to Aspose.Storage (or provide it directly in the request).  
2. **Call** the `PUT https://api.aspose.cloud/v4.0/cells/content/wordcase` endpoint with the desired `wordCaseType`.  
3. **Download** the processed workbook from the response or from the specified output location.

### Prerequisites
- An active Aspose.Cloud account with a valid **client‑id** and **client‑secret**.  
- OAuth 2.0 access token (see **Authentication** section below).  
- Supported file formats: XLSX, XLS, ODS, CSV, etc.  
- Required storage permission: `ReadWrite` (or higher) for the target folder.

### Authentication
The Change Word Case API uses **OAuth 2.0**. Obtain an access token by sending a `POST` request to  

```
https://api.aspose.cloud/connect/token
```

with your `client_id` and `client_secret` using the **client‑credentials** grant type. Include the token in all subsequent API calls:

```
Authorization: Bearer <access_token>
```

Tokens are typically valid for one hour; refresh them as needed.

## Overview

Converts text values in the chosen range to upper, lower, proper, or sentence case; formulas, formatting, and data‑validation remain untouched. Only string cells are affected—numbers, booleans, errors, and blanks are ignored.
...
| worksheet | String | Query | *(Optional)* The name of the worksheet where case conversion will be applied. If omitted, the operation applies to the first worksheet of the workbook. |
...
| region | String | Query | *(Optional)* Sets the locale for text‑case conversion rules, particularly relevant for language‑specific capitalization (e.g., `"en-US"`, `"tr-TR"`). |
...
```json
{
  "File": "Binary content of the updated workbook"
}
```
