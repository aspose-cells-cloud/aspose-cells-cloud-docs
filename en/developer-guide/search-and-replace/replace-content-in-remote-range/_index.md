...
To call the Replace Content in Remote Range API you must first obtain an access token.

1. **Request a token** – POST to `https://api.aspose.cloud/connect/token` with your **client_id** and **client_secret** (URL‑encoded form).  
2. **Receive the token** – The response contains `access_token`.  
3. **Include the token** – Add the header `Authorization: Bearer <access_token>` to every request, including the Replace Content call.

> **Note:** All calls must be made over **HTTPS**.
...
```bash
curl -X PUT "https://api.aspose.cloud/v4.0/cells/report.xlsx/worksheets/Sheet1/ranges/A1:D20/replace/content?searchText=OldValue&replaceText=NewValue&folder=MyFolder" \
