...
The Copy Folder API requires **OAuth 2.0** authentication.  
1. Request an access token from the Aspose Cloud authentication endpoint.  
...
* The source folder (`srcPath`) must already exist in the specified storage.  
* The token used must have the **Storage** scope.
...
```bash
curl -X PUT "https://api.aspose.cloud/v4.0/cells/storage/folder/copy/FolderA" \
     -H "Authorization: Bearer {access_token}" \
     -G \
     --data-urlencode "destPath=FolderA_Copy" \
     --data-urlencode "srcStorageName=MyStorage" \
     --data-urlencode "destStorageName=MyStorage"
```
...
1️⃣ **Obtain** an OAuth 2.0 token.  
2️⃣ **Build** the request URL, replacing `{srcPath}` with the source folder.  
3️⃣ **Add** required query parameters (`destPath`) and optional storage names.  
4️⃣ **Send** the `PUT` request with the `Authorization` header.  
5️⃣ **Verify** the response status is 200 and the body is `{}`.  
