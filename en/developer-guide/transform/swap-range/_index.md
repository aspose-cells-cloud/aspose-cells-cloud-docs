...
* **Aspose Cloud Account** – Sign up at the Aspose Cloud portal.  
* **OAuth 2.0 Access Token** – Obtain a token by posting your client‑id and client‑secret to  
  `POST https://api.aspose.cloud/connect/token`.  
* **Authorization Header** – Include the token in every request:  

  ```http
  Authorization: Bearer <access_token>
  ```

* **SDK Configuration (optional)** – If you use one of the Aspose.Cells Cloud SDKs, configure the client with your `client_id`, `client_secret`, and desired `basePath` (`https://api.aspose.cloud`).
...
<details>
<summary>How do I authenticate before calling the Swap Range API?</summary>
First obtain an OAuth 2.0 access token using your Aspose Cloud client‑id and client‑secret  
(`POST https://api.aspose.cloud/connect/token`). Include the token in the request header:  
...
<details>
<summary>What are the required parameters for swapping two ranges?</summary>
- **Spreadsheet** (file) – source workbook (required)  
- **worksheet1** (string) – source worksheet (required)  
- **range1** (string) – source range (required)  
- **worksheet2** (string) – target worksheet (required)  
- **range2** (string) – target range (required)  

Optional parameters: `outPath`, `outStorageName`, `region`, `password`.  
Both ranges must have identical dimensions.
</details>

<details>
<summary>Can I swap ranges that are on different worksheets?</summary>
Yes. Set `worksheet1` and `worksheet2` to the respective sheet names. The API works across worksheets as long as the ranges have the same size.
</details>

<details>
<summary>What happens if the dimensions of the two ranges differ?</summary>
The API returns **400 Bad Request** with a message such as “Range dimensions must match.”
</details>

<details>
<summary>How can I retrieve the swapped file after the operation?</summary>
If you provide `outPath` and `outStorageName`, the modified workbook is saved to that location. Otherwise, the response contains a file stream that you can download directly from the API response.
</details>