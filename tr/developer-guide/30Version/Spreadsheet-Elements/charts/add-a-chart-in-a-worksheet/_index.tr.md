...
| kategoriVerileri| sicim| sorgu|Kategori Eksen değerlerinin aralığını alır veya ayarlar. Bir hücre aralığı olabilir ("d1:e10" gibi).|
| isAutoGetSerialName| Boolean| sorgu| Doğru|
| başlık| sicim| sorgu| Grafik başlığının adını belirtir.|
| dosya| sicim| sorgu| Çalışma kitabı klasörü.|
| depolamaAdı| sicim| sorgu| depolama adı.|
| veriEtiketler| Boolean| sorgu| Doğru|
| veriEtiketleriKonumu| sicim| sorgu| Üstünde|
| pivotTableSheet| sicim| sorgu||
| pivotTabloAdı| sicim| sorgu||
 
 The[OpenAPI Spesifikasyonu](https://apireference.aspose.cloud/cells/#/Charts/PutWorksheetAddChart) herkesin erişebileceği bir programlama arayüzü tanımlar ve REST etkileşimlerini doğrudan bir web tarayıcısından gerçekleştirmenize olanak tanır.
